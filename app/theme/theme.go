...
func New(dir string) (*Theme, error) {
	...
	if err := theme.parseMeta(); err != nil {
		mylog.Warn("read theme metadata error %s", err.Error())
	}
...
func (th *Theme) parseMeta() error {
	file := path.Join(th.dir, vars.ThemeMetadataFile)
	th.metaFile = file
	var err error
	th.Meta, err = NewMetaFromFile(file)
	return err
}
	...
	err := filepath.Walk(th.dir, func(p string, fi os.FileInfo, err error) error {
		...
		for _, extension := range th.extensions {
			if ext == extension {
				...
				mylog.Trace("read theme file %s", p)
